...
public class Ex03 {
	public static void main(String[] args) {
		...
		int[]a=new int[500/7];
		System.out.println(a.length);		//71개 방을 가지고 있음 방번호 0 ~ 70
		//a배열 방의 각 방에 7의 배수 저장
		for(int i=0, j=7; i<a.length; i++,j+=7) {
			a[i]=j;		//배열a의 각 방에 7의 배수 저장
		}
		System.out.println(Arrays.toString(a));
		
		//배열 b : 9의 배수를 저장하는 배열 1 ~ 500까지
		int[]b=new int[500/9];
		System.out.println(b.length);		//55개 방을 가지고 있음 방번호 0 ~ 54
		for(int i=0, j=9; i<b.length; i++, j+=9) {
			b[i]=j;
		}
		System.out.println(Arrays.toString(b));
		
		System.out.println("=====54번 째 방의 값 출력======");
